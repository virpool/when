...
define(function() {
	...
	return function(basePromise) {
		var arrayPromise = basePromise.extend({
			...
			settle: function() {
				return this.constructor(this.then(resolveSettle)).all();

				function resolveSettle(array) {
					return array.map(function(x) {
						return basePromise(x).then(
							basePromise.toFulfilledState,
							basePromise.toRejectedState
						);
					});
				}
			},
			...
			reduce: function(reduceFunc) {
				...
				// rather than relying on basePromise here
				function resolveReduce(array) {
					args[0] = makeReducer(reduceFunc, array.length, basePromise);
