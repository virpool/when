...
define(function () {
	...
	return function makeCore(options) {
		...
		var enqueue, monitor, undef;

		enqueue = options.enqueue;
		monitor = options.monitor || {};

		return function makePromise(api) {
			return extendPromiseType(Promise, api);
		};

		function extendPromiseType(Base, api) {

			function PromiseType() {}

			PromiseType.prototype = extend(Base.prototype, api);

			lift.reject = reject;
			lift.promise = newPromise;
			lift.extend = extendPromiseType.bind(undef, PromiseType);
			...
			function newPromise(resolver) {
				return _promise(PromiseType, resolver);
			}
		}
		 ...
		 * @private
		 */
		function _promise(PromiseType, resolver, parent) {
			...
			function then(onFulfilled, onRejected, onProgress) {
				var next = _promise(PromiseType, function(resolve, reject, notify) {
		...
		function coerce(x) {
			...
			return _promise(Promise, function(resolve, reject, notify) {
				enqueue(function() {
					try {
