...
define(function() {
	...
	function makePromise(environment) {
		...
		Promise.prototype.then = function(onFulfilled, onRejected, onProgress) {
			...
			p._handler.receiver = this._handler.receiver;
		...
		DeferredHandler.prototype.join = function(handler) {
			handler.receiver = this.receiver;
		...
		FollowingHandler.prototype.when = function(resolve, notify, f, r, u) {
			...
			// to temporarily allow the followed handler to use this
			// handler's receiver. There really has to be a better way.
			var original = this.promise._handler.receiver;
			this.promise._handler.receiver = this.receiver;
			this.promise._handler.when(resolve, notify, f, r, u);
			this.promise._handler.receiver = original;
		};
		...
		FulfilledHandler.prototype.when = function(resolve, _, f) {
			tasks.enqueue(new FulfillTask(resolve, f, this.value, this.receiver));
		};
		...
		RejectedHandler.prototype.when = function(resolve, _, __, r) {
			tasks.enqueue(new RejectTask(resolve, r, this.value, this.receiver));
		};
		...
		ProgressHandler.prototype.when = function(_, notify, __, ___, u) {
			try {
				notify(typeof u === 'function' ? u.call(this.receiver, this.value) : this.value);
			} catch(e) {
				notify(e);
			}
		};
		 ...
		 * @constructor
		 */
		function FulfillTask(resolve, map, value, receiver) {
		...
		FulfillTask.prototype.run = function() {
			...
			var map = t.map;
			try {
				t.resolve(typeof map === 'function' ? map.call(t.receiver, t.value) : t.value);
			} catch(e) {
		 ...
		 * @constructor
		 */
		function RejectTask(resolve, map, value, receiver) {
			this.resolve = resolve;
			this.map = map;
			this.value = value;
			this.receiver = receiver;
		...
		RejectTask.prototype.run = function() {
			...
			try {
				t.resolve(map.call(t.receiver, t.value));
			} catch(e) {
				doReject(t.resolve, e);
			}
		};
